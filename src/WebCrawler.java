...
public class WebCrawler {
	...
	/*crawl goes to all pages reachable from originalUrl under
	 * the domain of originalUrl and prints out the static assets (images, 
	 * javascript, stylesheets) for each page in JSON format listing the URLs of 
	 * every static asset on that page.*/
	public void crawl(){
        ...
        while(!queue.isEmpty()){
        	...
        	try{
    			...
    			System.out.println(formatAssetsForPage(staticAssets, currentDocument));
	...
	/*enqueueAllReachableLinks finds the href attribute of all the
	 * hyperlinks (<a> tag) in currentDocument.
	 * Then it checks that the url belongs to the same domain as originalUrl. 
	 * Then we check to make sure that we have not been to this url before.
	 * If we have not, then we enqueue this url to the queue of pages to visit 
	 * and add the url to our set of explored urls.*/
	...
	/*Returns true if url is present in the queue and
	 * therefore the url is also present in the set of explored urls.*/
	boolean isExplored(String url){
		return explored.contains(url);
	}
	
	/*Formats a string of all the static assets found in currentDocument
	 * so that this string can be easily printed out by the calling method.*/
	String formatAssetsForPage(ArrayList<String> staticAssets, Document currentDocument){
		String assets= "\t{\n";
	...
	/*endWithSlash ensures that url ends with a slash. We use this method
	 *to make sure that all the urls that we store in the set, explored, have the
	...
	/*Sets the originalUrl, which we store need to store in order to
	 *ensure that the links that we find are under the domain of the
	 *originalUrl.*/
